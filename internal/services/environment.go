...
func (e *EnvironmentService) InitializeGitRepository(path string) error {
	...
	if _, err := exec.LookPath("git"); err != nil {
		return fmt.Errorf("%w: git is not installed or not found in PATH (install from https://git-scm.com/downloads)", models.ErrToolNotFound)
	}
...
func (e *EnvironmentService) ValidateProjectPath(path string, isHere bool) error {
	...
	if !isHere && exists {
		return fmt.Errorf("%w: directory already exists: %s", models.ErrProjectAlreadyExists, path)
	}
...
func (e *EnvironmentService) WaitForConnectivity(timeout time.Duration) error {
	...
	return fmt.Errorf("%w: timeout waiting for internet connectivity (check your network connection)", models.ErrInternetNotAvailable)
}
