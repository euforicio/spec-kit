...
func (f *FeatureService) SetupPlan() (*models.FeaturePlanResult, error) {
	...
	templatePath := filepath.Join(repoRoot, "templates", "plan-template.md")
	planFile := filepath.Join(featureDir, "plan.md")

	if exists, _ := f.filesystem.FileExists(templatePath); exists {
		if err := f.filesystem.CopyFile(templatePath, planFile); err != nil {
			return nil, fmt.Errorf("failed to copy plan template: %w", err)
		}
	}
