...
func (e *Environment) validatePlatform() error {
	if e.Platform == "" {
		return fmt.Errorf("%w: platform cannot be empty", ErrToolNotFound)
	}
	...
	return fmt.Errorf("%w: unsupported platform '%s', supported platforms: %v", ErrToolNotFound, e.Platform, SupportedPlatforms)
}

// validateWorkingDir validates the working directory
func (e *Environment) validateWorkingDir() error {
	if e.WorkingDir == "" {
		return fmt.Errorf("%w: working directory cannot be empty", ErrToolNotFound)
	}
...
func (e *Environment) validateGitConfig() error {
	if e.GitConfig.Version == "" {
		return fmt.Errorf("%w: git version cannot be empty when git is available", ErrToolVersionUnsupported)
	}
